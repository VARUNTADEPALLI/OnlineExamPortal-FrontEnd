...
function ForgptPasswordComponent(props) {
...
// {
//     axios.get("http://localhost:9025/getUser/"+q)
//     .then(response => 
...
// }
    ...
    const changeHandler = (e) =>
    {
            ...
            if(sum===0)
            {
                ...
                console.log(data);
                axios.put("http://localhost:9025/updateUserDetails/"+id,data)
                .then(response =>{console.log(response);}).catch(err => {alert(err);err++;});
                if(err===0)
                {
                    alert("suuccessful updte");
                    history("/login");
                }
                else
                {
...
export default ForgptPasswordComponent;